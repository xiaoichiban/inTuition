...
// you just save the sockets its-self.
...
if (!ctype_alnum(session_id()) 
	|| !preg_match('/^(?:[a-z0-9_-]|\.(?!\.))+$/iD', session_id())) 
	{	die();	}
...
if (!isset($_GET['eventSource'])) { 
	//
	// show all the HTML CSS and Javascript
	//
?>
...
<!DOCTYPE html>
    ...
    navigator.mediaDevices.getUserMedia({audio: true, video: true }).then(function (stream) {
            ...
            localStream = stream;
            try {ws = new EventSource(URL);}
			catch(e) { console.error("Could not create eventSource ",e);}
		...
		// Websocket-hack: onmessage is extended for receiving 
            ...
            // Go show myself
            localVideo.addEventListener('loadedmetadata', function () {publish('client-call', null)});
			
        }).catch(function (e) {console.log("Problem while getting audio video stuff ",e);});
    ...
    function onsinglemessage(data) {
        ...
        switch (package.event) {
			...
			case 'client-call':
				...
				).then(function (desc) {pc.setLocalDescription(desc
				).then(function () { publish('client-offer', pc.localDescription); }
                ).catch(function (e) {console.log("Problem with publishing client offer"+e); });
                }).catch(function (e) { console.log("Problem while doing client-call: "+e); });
                break;
                    ...
                    if (!answer) {
                        pc.createAnswer(function (desc) {
                            pc.setLocalDescription(desc, function () {
                                ...
                                }, function(e){ console.log("Problem getting client answer: ",e); });
                            },function(e){ console.log("Problem while doing client-offer: ",e); });
                        answer = 1;
                    }
                }, function(e){ console.log("Problem while doing client-offer2: ",e); });
                break;
...
<?php
	...
	// regular eventSource poll which is loaded every few seconds
	...
	// shortens the 3 seconds to 1 sec
}
